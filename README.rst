...
In newer ES6 code, when using the ``import`` directive, index.js files become more common. So you usually apply the ``import`` directive to some directory, but actually
you mean the index.js file inside this directory. As the filename 'index' will appear in the JS path generated by Sphinx, it might not be wishful to always have it displayed
in the resulting documentation. So we introduced the option ``:skip-index:`` which suppresses an 'index' part inside the JS path. You will get ``myModule.myFunc`` instead of
``myModule.index.myFunc``, if you apply this option to any directives.
